    ...
    def __init__(self, is_training=True):
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope("encoder"):
                ...
                ## Positional Encoding
                pe1,pe2,pe3= embedding_en(tf.tile(tf.expand_dims(tf.range(tf.shape(self.x1)[1]), 0), [tf.shape(self.x1)[0], 1]),
                ...
                for i in range(hp.num_blocks):
                    with tf.variable_scope("num_blocks_{}".format(i)):
                        ### Multihead Attention
                        self.enc1,self.enc2,self.enc3 = multihead_attention_en(queries1=self.enc1,queries2=self.enc2,queries3=self.enc3,
                                                        keys1=self.enc1,keys2=self.enc2,keys3=self.enc3,
                                                        num_units=hp.hidden_units, 
    ...
    with sv.managed_session() as sess:
        for epoch in range(1, hp.num_epochs+1): 
            ...
            for step in range(g.num_batch):
                sess.run(g.train_op)
                if step%20==0:
                    print('epoch:'+str(epoch)+'----'+'step:'+str(step)+'----loss:'+str(sess.run(g.mean_loss))+'----acc:'+str(sess.run(g.acc)))
